...
if (empty($sortfield)) { $sortfield = 'c.date_contrat'; } // Orden por fecha contrato
...
$search_amount = GETPOST('search_amount', 'alpha'); // Nuevo filtro importe
...
// -------------------------------------------------------------------
// --- SQL ACTUALIZADO (1:N) ---
$sql = "SELECT c.rowid, c.ref, c.date_contrat, c.statut, c.total_ttc, ";
...
if ($search_status != '' && $search_status != -1) $sql .= " AND c.statut = " . $search_status;
...
print_liste_field_titre("Fecha", $_SERVER["PHP_SELF"], "c.date_contrat", "", $param, 'align="center"', $sortfield, $sortorder);
...
// Estados Contrato: 0=Borrador, 1=Validado, 2=Cerrado
...
if ($resql && $db->num_rows($resql) > 0) {
    ...
    print '<tr><td colspan="6" class="opacitymedium">No se encontraron contratos.</td></tr>';
}
