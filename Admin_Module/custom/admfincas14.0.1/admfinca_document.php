...
/**
 *  \file       admfinca_document.php
 *  \ingroup    admfincas
 *  \brief      Tab for documents linked to Admfinca
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->admfincas->multidir_output[$object->entity ? $object->entity : $conf->entity] . "/admfinca/" . get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = !empty($user->rights->admfincas->admfinca->write); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
}
...
// Header
// ------
$title = $langs->trans("Admfinca") . " - " . $langs->trans("Files");
//$title = $object->ref." - ".$langs->trans("Files");
$help_url = '';
//Example $help_url='EN:Module_Third_Parties|FR:Module_Tiers|ES:Empresas';
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-admfincas page-card_document');

// Show tabs
$head = admfincaPrepareHead($object);

print dol_get_fiche_head($head, 'document', $langs->trans("Admfinca"), -1, $object->picto);
...
$param = '&id=' . $object->id;
//$relativepathwithnofile='admfinca/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'admfinca/' . dol_sanitizeFileName($object->ref) . '/';
