...
/**
 *  \file       admfinca_note.php
 *  \ingroup    admfincas
 *  \brief      Tab for notes on Admfinca
 */
...
$langs->loadLangs(array("admfincas@admfincas", "companies"));
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->admfincas->multidir_output[empty($object->entity) ? $conf->entity : $object->entity] . "/" . $object->id;
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = !empty($user->rights->admfincas->admfinca->write);
...
$title = $langs->trans('Admfinca') . ' - ' . $langs->trans("Notes");
//$title = $object->ref." - ".$langs->trans("Notes");
$help_url = '';
//$help_url='EN:Customers_Orders|FR:Commandes_Clients|ES:Pedidos de clientes';

llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-admfincas page-card_notes');
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'note', $langs->trans("Admfinca"), -1, $object->picto);
